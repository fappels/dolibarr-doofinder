...
?>
	...
	if (! empty($conf->product->enabled) || ! empty($conf->service->enabled))
	{
		...
		if (empty($senderissupplier))
		{
			// allow price edit before adding, transfer price_by_qty
			/*$ajaxoptions=array(
					'update' => array('qty'=>'qty','remise_percent' => 'discount', 'price_ht' => 'price_ht', 'tva_tx' => 'tva_tx', 'min_qty' => 'min_qty', 'max_qty' => 'max_qty')
			);
			if ($conf->global->ENTREPOT_EXTRA_STATUS)
			{
				// hide products in closed warehouse, but show products for internal transfer
				$form->select_produits(GETPOST('idprod'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, 1, 2, '', 1, $ajaxoptions,$buyer->id, '1', 0, '', 0, 'warehouseopen,warehouseinternal', GETPOST('combinations', 'array'));
			}
			else
			{
				$form->select_produits(GETPOST('idprod'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, 1, 2, '', 1, $ajaxoptions,$buyer->id, '1', 0, '', 0, '', GETPOST('combinations', 'array'));
			}*/
			dol_include_once('/doofinder/class/doofinder.class.php');
			$doofinderApi = new Doofinder($this->db);
			$doofinderApi->connect();
			$hashid = $doofinderApi->getSearchApiHashid();
			$apikey = $doofinderApi->getSearchApiKey();
			if ($object->thirdparty->price_level > 1) {
				$price_level = $object->thirdparty->price_level;
			} else {
				$price_level = 1;
			}
			print '<!-- doofinder template -->';
			print '<input type="text" id="search_idprod">';
			print '<input type="hidden" name="idprod" id="idprod">';
			print '<script type="text/x-mustache-template" id="my-results-template">';
			$doofinder_template = file_get_contents($dolibarr_main_document_root_alt.'/doofinder/core/tpl/doofinder_template.mustache');
			print $doofinder_template;
			print '</script>';
			include 'doofinder.script.php';
		}
		else
		{
	...
	print '<input id="dp_ref" name="dp_ref" class="flat maxwidth100">';
	...
	<td class="nobottom linecolqty" align="right">
		<input type="text" size="2" name="qty" id="qty" class="flat right" value="<?php echo (isset($_POST["qty"])?GETPOST("qty",'alpha',2):1); ?>">
...
jQuery(document).ready(function() {
	...
	jQuery("#qty").hover(function() {
		var qty = parseFloat(jQuery("#qty").val().replace(',', '.'));
			minQty = parseFloat(jQuery("#min_qty").val().replace(',', '.'));
			maxQty = parseFloat(jQuery("#max_qty").val().replace(',', '.'));
			unitPrice = parseFloat(jQuery("#price_ht").val().replace(',', '.'));
		
		jQuery("#qty").prop('title', '');
		if (qty > 0 && unitPrice > 0) {
			if (minQty && qty < minQty) {
				jQuery("#qty").prop('title', "Qté "+qty+" trop bas pour le prix unitaire sélectionné.\nLa Qté doit etre "+minQty+" ou plus.\nLes prix unitaires seront calculés en fonction de la quantité donnée\nlors de la soumission de la ligne.");
			} else if (maxQty && qty >= maxQty) {
				jQuery("#qty").prop('title', "Qté "+qty+" trop élevé pour le prix unitaire sélectionné.\nLa Qté doit etre moin de "+maxQty+". \nLes prix unitaires seront calculés en fonction de la quantité donnée\nlors de la soumission de la ligne.");
			} else {
				jQuery("#qty").prop('title', "Prix unitaire " + unitPrice + (minQty?" pour Qté minimal: "+minQty:"")+(maxQty?" Qté maximal "+maxQty:""));
			}
		}
	});
...
function setforpredef() {
	...
	jQuery("#dp_ref").show();
	jQuery("#dp_ref").prop('readonly', true);
}
